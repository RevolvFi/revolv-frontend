...
async function handleTransaction(tx: TransactionResponse): Promise<void> {
  addTransaction({
    ...
    type: 'tx',
    action: 'unlock',
    summary: `${fNum(props.totalRvlvTokens, FNumFormats.token)} RVLV`,
    details: {
      totalRvlvTokens: props.totalRvlvTokens,
    },
  });
...
async function submit() {
  try {
    ...
    // For RVLV direct unlocking, we need to call the appropriate unlock function
    // This might be different from the regular veBAL unlock
